...
namespace Dwarf
{
    ...
    CLASS Debug : public DebugBase
    {
        struct RangeEntry
        {
            ...
            RangeEntry() : upper_(0) { }
            RangeEntry(Dwarf_Addr upper, std::shared_ptr<CompileUnit> unit)
                : upper_(upper)
                , unit_(unit)
            { }
        };
    ...
    public:
        typedef std::vector<std::shared_ptr<CompileUnit> > UnitList;
        typedef std::vector<std::shared_ptr<Datum> > Data;
    ...
    #else
    #error no hash_map!
        typedef std::map<Dwarf_Off, std::shared_ptr<Die> > DieCache;
    #endif
    ...
    public:
         ...
         */
        std::shared_ptr<CompileUnit> lookup_unit(Dwarf_Addr) const;

        /**
         * Lookup compilation unit by file name
         */
        std::shared_ptr<CompileUnit> lookup_unit(const char*) const;
         ...
         * by linkage name
         */
        std::shared_ptr<Function> lookup_function(
            Dwarf_Addr,
            const char* linkage = NULL) const;

        std::shared_ptr<Type> lookup_type(const char*, int level = 0) const;
        std::shared_ptr<Type> lookup_type_by_decl(const Type&) const;
         ...
         * function parameters
         */
        std::shared_ptr<Type> lookup_type_by_ctor(const char*) const;

        static std::string infer_ctor_name(const char*);
        std::shared_ptr<Type> lookup_type_by_ctor(const std::string&) const;

        std::shared_ptr<Die> get_object(
            Dwarf_Off,
        ...
        std::shared_ptr<CompileUnit> get_compile_unit(Dwarf_Die,
    ...
    private:
        ...
        std::shared_ptr<CompileUnit> lookup_unit_by_arange(Dwarf_Addr) const;
    ...
    /*  mutable Cache<Dwarf_Var>* varCache_; */
        ...
        mutable std::shared_ptr<Unwind> unwind_; /* frame unwind info */
