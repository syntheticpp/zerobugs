...
FlPackLayout::FlPackLayout(ui::Controller& c, int x, int y, int w, int h)
    : ui::Layout(c)
    , group_(new Fl_Tile(x, y + Const::menubar_height, w, h - Const::menubar_height - Const::statbar_height))
    , code_(nullptr)
    , bottom_(nullptr)
    , right_(nullptr)
{
    Fl_Tile* tile = new Fl_Tile(0, y + Const::menubar_height, w, code_height());
    tile->type(Fl_Pack::VERTICAL);
    tile->box(FL_DOWN_BOX);

    // area where source or assembly code is displayed
    auto pack = new Fl_Group(x, y + Const::menubar_height, w - Const::thread_regs_width, code_height());
    ...
    auto g = new Fl_Group(x + code_->w(), y + Const::menubar_height, Const::thread_regs_width, tile->h());
    g->box(FL_DOWN_BOX);
